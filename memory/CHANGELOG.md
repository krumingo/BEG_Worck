...
### Fixed
- **Popup "Потвърди" сега обновява главната селекция.** Преди: popup-ът смяташе сумата
  локално (42.63€·3.0ч), но при затваряне главните totals оставаха 0 EUR · 0 дни.
  Сега: при натискане "Потвърди" — деня се добавя в `selectedDays`, employee-то в
  `selectedEmps`, totals се преизчисляват от само избраните report_id-та.
- **getDayValue/getDayHours четат selectedReportIds.** Ако popup-ът е избрал
  конкретни отчети → totals използват само техните стойности (не целия dc.value).
  Ако няма popup избор → fallback на dc.selectable_value (само неплатеното).
- **Auto-check при отваряне на popup.** Когато mixed клетка се отвори, popup-ът
  автоматично маркира всички selectable отчети — user вижда веднага "ето кое ще се плати"
  и може да изключи отделни с unmark.
- **Визуален индикатор на частичен избор.** Когато user е избрал по-малко от всички
  selectable отчети, синята секция на клетката показва "2/3 избрани · 5.0ч · 71".

### Files changed
- `frontend/src/pages/PayRunsPage.js` — само този файл

### Not changed
- Backend: 0 промени
- `payroll_sync.py`, `report_normalizer.py`, `pay_runs.py`, `settings.py` — непокътнати
- P0-1 защита (FIX 1 + FIX 5) — активна и непокътната
- AllReportsPage.js, EmployeeDetailPage.js, GroupedReportsTable.js — без промяна

### Flow на човешки
1. Отваряш mixed клетка → popup отваря с pre-checked всички неплатени.
2. Махаш отметка от 1 отчет → popup-ът показва "2/3 избрани · сума".
3. Натискаш "Потвърди" → главните totals се обновяват със същата сума.
4. Натискаш "Откажи" → нищо не се променя в главните totals.
5. Backend получава `selected_report_ids` с точните избрани → маркира само тях.

### Acceptance tests
1. Mixed клетка (2 платени + 1 неплатен): отваряш → виждаш 1/1 избрано → Потвърди →
   главните totals показват сумата на неплатения отчет.
2. Mixed клетка (1 платен + 2 неплатени): отваряш → виждаш 2/2 избрани → махаш 1 →
   виждаш "1/2 избрани" + новата сума → Потвърди → totals се обновяват.
3. Натискаш "Изчисти" → попап picks се пазят, но клетката не е в selectedDays → 0 EUR.
4. P0-1 guard работи — backend винаги пропуска paid/batched, дори ако нещо излезе нагоре.
...
- 50+ Pydantic модела — вече в app/models/
- Auth helpers — вече в app/deps/auth.py и app/deps/modules.py
- log_audit() — вече в app/utils/audit.py
- Дублирани константи ROLES, MODULES — вече в app/constants.py

### Moved (нови файлове)
- app/core/mobile_constants.py — MOBILE_MODULES, MOBILE_ACTIONS, MOBILE_FIELDS
- app/core/stripe_config.py — STRIPE_API_KEY, STRIPE_MOCK_MODE
- app/core/seed.py — seed_data() функция

### Fixed
- Latent bug: run_reminder_jobs import fix

---

## [M16.7] - 2026-05-05 — Премахната миграционна система

### Removed
- backend/migrate_to_production.py — production data migration script (no longer needed)
- backend/app/migrations/m15_fifo_migration.py — FIFO migration script
- backend/app/migrations/ directory (now empty)
- backend/server_backup.py — 236KB legacy backup file
- Startup hook `from migrate_to_production import migrate` in server.py

### Added
- Auto-call of ensure_indexes() for warehouse_batches at startup (FIFO performance fix)

### Notes
- Sub-project "migration" logic in routes/projects.py is a different feature (Cyrillic А/Б/В split) — NOT touched
- All existing tests pass (26/26 FIFO + Sales)
