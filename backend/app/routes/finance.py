...
async def get_next_number(
    ...
    """Preview next invoice number — uses same logic as create"""
    ...
    settings = await get_invoice_settings(user["org_id"])
    prefix = settings.get("issued_prefix", "INV") if direction == "Issued" else settings.get("received_prefix", "BILL")

    try:
        num = int(next_number.replace(f"{prefix}-", "").lstrip("0") or "0")
    except ValueError:
        num = 0
...
async def create_other_income(data: dict, user: dict = Depends(require_m5)):
    """Documentless cash income (proforma / deposit / cash receipt without a regular invoice).
    Creates a finance_payments Inflow into Каса/Банка tagged category='Други приход'. It is NOT
    P&L revenue (revenue comes from invoices) — it only affects cash."""
    if not finance_permission(user):
        raise HTTPException(status_code=403, detail="Insufficient permissions")
    org = user["org_id"]
    project_id = data.get("project_id")
...
# ── Financial Pulse Endpoints ──────────────────────────────────────

@router.get("/finance/aging-report")
async def aging_report(user: dict = Depends(require_m5)):
    ...
    buckets = {"not_due": [], "1_30": [], "31_60": [], "60_plus": []}
    client_totals = {}

    for inv in invoices:
        remaining = inv.get("remaining_amount") or inv.get("total", 0)
        ...
        try:
            ...
            due_dt = dt.strptime(due[:10], "%Y-%m-%d") if due else dt.now()
            today_dt = dt.strptime(today, "%Y-%m-%d")
            days_diff = (today_dt - due_dt).days
        except (ValueError, TypeError):
            days_diff = 0

        row = {"invoice_no": inv.get("invoice_no"), "client_name": client,
               "amount": remaining, "due_date": due, "days_overdue": max(0, days_diff)}
        ...
        if days_diff <= 0:
            ...
            buckets["60_plus"].append(row)

        # Client aggregation
        if client not in client_totals:
            client_totals[client] = {"client_name": client, "not_due": 0, "1_30": 0, "31_60": 0, "60_plus": 0, "total": 0}
        bucket_key = "not_due" if days_diff <= 0 else "1_30" if days_diff <= 30 else "31_60" if days_diff <= 60 else "60_plus"
        client_totals[client][bucket_key] += remaining
        client_totals[client]["total"] += remaining

    overdue_all = buckets["1_30"] + buckets["31_60"] + buckets["60_plus"]
    oldest = max((r["days_overdue"] for r in overdue_all), default=0)

    return {
        "buckets": {k: {"count": len(v), "total": round(sum(r["amount"] for r in v), 2), "invoices": v[:5]} for k, v in buckets.items()},
        "totals": {
            "total_count": len(invoices),
            "total_amount": round(sum(inv.get("remaining_amount", inv.get("total", 0)) for inv in invoices), 2),
            "overdue_count": len(overdue_all),
            "overdue_amount": round(sum(r["amount"] for r in overdue_all), 2),
            "oldest_overdue_days": oldest,
        },
        "by_client": sorted(client_totals.values(), key=lambda x: x["total"], reverse=True)[:10],
    }


@router.get("/finance/upcoming-payments")
async def upcoming_payments(days: int = 5, user: dict = Depends(require_m5)):
    ...
    def enrich(inv):
        from datetime import datetime as dt
        try:
            days_until = (dt.strptime(inv.get("due_date", "")[:10], "%Y-%m-%d") - dt.strptime(today, "%Y-%m-%d")).days
        except (ValueError, TypeError):
            days_until = 0
        return {
            "invoice_no": inv.get("invoice_no"),
            "client_name": inv.get("counterparty_name", ""),
            "amount": inv.get("remaining_amount") or inv.get("total", 0),
            "due_date": inv.get("due_date"),
            "days_until_due": days_until,
        }

    return {"receivables": [enrich(r) for r in receivables], "payables": [enrich(p) for p in payables]}
