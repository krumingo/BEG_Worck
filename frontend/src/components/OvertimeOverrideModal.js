...
export default function OvertimeOverrideModal({ open, onOpenChange, blocked = [], onSubmit }) {
  ...
  const validate = (id) => {
    ...
    if (Math.abs(reg + ot - h) > 0.01) errors.push(`Сборът трябва да е равен на ${h}ч`);
    if (reg < 0) errors.push("Редовни часове >= 0");
    if (ot < 0) errors.push("Извънредни часове >= 0");
    const coef = parseFloat(r.overtime_coefficient) || 0;
    if (!coef || coef <= 1) errors.push("Коефициент трябва да е > 1");
    if (!r.reason?.trim()) errors.push("Причината е задължителна");
    return errors;
  };
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Извънредни часове — Admin Override
          </DialogTitle>
        </DialogHeader>
        <p className="text-xs text-muted-foreground mb-4">
          Следните отчети надвишават 8ч/ден. Разпределете часовете и посочете причина.
        </p>
              ...
              <div key={b.id} className={`rounded-xl border p-4 space-y-3 ${hasErr ? "border-red-500/30" : "border-border"}`}>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <Badge variant="outline" className="text-[9px]"><Clock className="w-3 h-3 mr-1" />Ден: {b.current_hours}ч</Badge>
                    <Badge variant="outline" className="text-[9px] bg-amber-500/10 text-amber-400 border-amber-500/30">Отчет: {b.report_hours}ч</Badge>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <Label className="text-xs">Редовни часове</Label>
                  ...
                  <div className="space-y-1">
                    <Label className="text-xs">Извънредни часове</Label>
                ...
                {/* Coefficient */}
                <div className="space-y-1">
                  <Label className="text-xs">Коефициент за извънреден труд</Label>
                  <div className="flex gap-2">
                    <Select value={r.coef_mode || ""} onValueChange={v => {
                      ...
                      <SelectTrigger className="w-[140px] h-9"><SelectValue placeholder="Изберете..." /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="1.5">1.5×</SelectItem>
                        <SelectItem value="2.0">2.0×</SelectItem>
                        <SelectItem value="custom">Друг</SelectItem>
                      </SelectContent>
                    </Select>
                    {r.coef_mode === "custom" && (
                      <Input type="number" value={r.overtime_coefficient || ""} onChange={e => update(b.id, "overtime_coefficient", e.target.value)}
                        min="1.01" step="0.1" placeholder="напр. 1.75" className="h-9 w-24 font-mono" />
                    )}
                  </div>
                </div>
                ...
                {/* Reason */}
                <div className="space-y-1">
                  <Label className="text-xs">Причина (задължителна)</Label>
                  <Textarea value={r.reason || ""} onChange={e => update(b.id, "reason", e.target.value)}
                    placeholder="Спешен график, допълнителна смяна..." className="text-xs min-h-[50px]" />
                </div>
        ...
        <div className="flex justify-end gap-2 mt-4">
          ...
          <Button onClick={handleSubmit} disabled={!allValid || submitting} className="bg-amber-500 hover:bg-amber-600 text-black">
            ...
            Одобри с override ({blocked.length})
          </Button>
        </div>
      </DialogContent>
    </Dialog>
