...
import { MapPin, Calendar, History, Loader2, Package, Wrench, Shield } from "lucide-react";
...
export default function EmployeeAssetsTab({ userId }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const r = await API.get(`/assets/units?location_type=employee&location_id=${userId}&page_size=300`);
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="rounded-lg bg-muted/40 p-3">
          <p className="text-xs text-muted-foreground flex items-center gap-1"><Shield className="w-3.5 h-3.5" />ЛПС</p>
          <p className="text-2xl font-semibold mt-0.5">0</p>
        </div>
      </div>

      {units.length === 0 ? (
        <div className="text-center py-12 text-muted-foreground text-sm">Няма зачислени активи на този човек.</div>
      ) : (
        <div className="rounded-xl border border-border overflow-hidden">
          <div className="grid grid-cols-[2fr_1.2fr_1.1fr_0.7fr] gap-3 px-4 py-2.5 border-b border-border text-xs text-muted-foreground">
            <span>Актив</span><span>Тип</span><span>Зачислен на</span><span className="text-right">История</span>
          </div>
          {units.map((u) => (
