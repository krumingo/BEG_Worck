  ...
  return (
    <button onClick={onNavigateFinance} className="bg-gray-800/50 border border-gray-700 rounded-lg p-4 text-left hover:bg-gray-700/30 transition-colors w-full" data-testid="project-finance-card">
      ...
      <div className="grid grid-cols-3 gap-2 text-xs">
        ...
        <div><p className="text-gray-400">Фактурирано</p><p className="font-mono text-white">{fmt(data.total_revenue)}</p></div>
        <div><p className="text-gray-400">Получено</p><p className="font-mono text-emerald-400">{fmt(data.total_revenue)}</p></div>
