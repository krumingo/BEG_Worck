      ...
      <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
        ...
        <TabsContent value="overview" className="space-y-4 mt-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            {/* Card: Финансов статус */}
            <ProjectFinanceCard projectId={projectId} onNavigateFinance={() => handleTabChange("finance")} />
          ...
          {pendingReports.length > 0 && (
            <div className="bg-amber-500/5 border border-amber-500/30 rounded-lg p-4" data-testid="team-pending-approval">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-sm font-semibold text-amber-400">{pendingReports.length} отчета за одобрение</h3>
                {bulk.count > 0 && (
                  <div className="ml-auto flex items-center gap-2">
                    <span className="text-xs text-muted-foreground">Маркирани: {bulk.count}</span>
                    <Button size="sm" className="h-7 text-[10px] bg-emerald-600 hover:bg-emerald-700" disabled={bulkLoading} onClick={async () => {
                      setBulkLoading(true);
                      try {
                    ...
                    <Button size="sm" variant="outline" className="h-7 text-[10px] text-red-400 border-red-500/30" disabled={bulkLoading} onClick={async () => {
                    ...
                    <Button size="sm" variant="ghost" className="h-7 text-[10px]" onClick={bulk.clear}>Изчисти</Button>
                  </div>
                )}
              </div>
              <div className="space-y-2">
                {/* Header checkbox */}
                <label className="flex items-center gap-2 text-[10px] text-muted-foreground cursor-pointer px-3">
                  <input type="checkbox" checked={bulk.count === pendingReports.length && pendingReports.length > 0} onChange={() => bulk.toggleAll(pendingReports.map(r => r.id))} className="rounded" />
                  Избери всички
                </label>
                {pendingReports.slice(0, 20).map(r => (
                  <div key={r.id} className={`flex items-center justify-between bg-card/50 rounded-lg px-3 py-2 border ${bulk.isSelected(r.id) ? "border-primary/50 bg-primary/5" : "border-border/50"}`}>
                    <div className="flex items-center gap-3 min-w-0">
                      <input type="checkbox" checked={bulk.isSelected(r.id)} onChange={() => bulk.toggleId(r.id)} className="rounded" />
