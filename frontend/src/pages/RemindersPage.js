  ...
  return (
    <div className="p-8 max-w-[1200px]" data-testid="reminders-page">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">{t("reminders.title")}</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6" data-testid="reminder-cards">
        <div className="stat-card animate-in" data-testid="missing-att-card">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">{t("reminders.missingAttendance")}</span>
        ...
        <div className="stat-card animate-in" style={{ animationDelay: "80ms" }} data-testid="missing-rep-card">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">{t("reminders.missingReports")}</span>
        ...
        <div className="stat-card animate-in" style={{ animationDelay: "160ms" }} data-testid="reminders-sent-card">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">{t("reminders.remindersSent")}</span>
        ...
        <Tabs defaultValue="attendance" className="w-full" data-testid="reminder-tabs">
          <TabsList className="bg-card border border-border">
            <TabsTrigger value="attendance" data-testid="tab-missing-att">
              <CalendarCheck className="w-4 h-4 mr-2" /> {t("reminders.missingAttendance")} ({missingAtt.length})
            </TabsTrigger>
            <TabsTrigger value="reports" data-testid="tab-missing-rep">
              <FileText className="w-4 h-4 mr-2" /> {t("reminders.missingReports")} ({missingRep.length})
            </TabsTrigger>
            <TabsTrigger value="logs" data-testid="tab-logs">
              <Bell className="w-4 h-4 mr-2" /> {t("reminders.reminderLog")} ({logs.length})
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="attendance" className="mt-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-foreground">{t("reminders.usersNoAttendance")}</h3>
              {missingAtt.length > 0 && (
                <Button size="sm" onClick={() => handleSendReminders("MissingAttendance", missingAtt.map((m) => m.user_id))} disabled={sending} data-testid="send-att-reminders">
                  ...
                  {t("reminders.remindAll")} ({missingAtt.length})
                </Button>
              )}
            </div>
            <div className="rounded-xl border border-border bg-card overflow-hidden">
              <Table>
                ...
                <TableBody>
                  {missingAtt.length === 0 ? (
                    <TableRow><TableCell colSpan={3} className="text-center py-8 text-muted-foreground">{t("reminders.everyoneCheckedIn")}</TableCell></TableRow>
                  ) : (
                    missingAtt.map((m) => (
                        ...
                        <TableCell className="text-right">
                          <div className="flex items-center justify-end gap-1">
                            <Button size="sm" variant="outline" onClick={() => handleSendReminders("MissingAttendance", [m.user_id])} disabled={sending}>
                              <Send className="w-3.5 h-3.5 mr-1" /> {t("reminders.remind")}
                            </Button>
                            <Button size="sm" variant="ghost" onClick={() => openExcuse(m.user_id, "MissingAttendance")}>
                              <Shield className="w-3.5 h-3.5 mr-1" /> {t("reminders.excuse")}
                            </Button>
                          </div>
                        </TableCell>
                      </TableRow>
          ...
          <TabsContent value="reports" className="mt-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-foreground">{t("reminders.usersNoReport")}</h3>
              {missingRep.length > 0 && (
                <Button size="sm" onClick={() => handleSendReminders("MissingWorkReport", missingRep.map((m) => m.user_id), missingRep[0]?.project_id)} disabled={sending} data-testid="send-rep-reminders">
                  {sending ? <Loader2 className="w-4 h-4 animate-spin mr-1" /> : <Send className="w-4 h-4 mr-1" />}
                  {t("reminders.remindAll")} ({missingRep.length})
                </Button>
              )}
            </div>
            <div className="rounded-xl border border-border bg-card overflow-hidden">
              <Table>
                ...
                <TableBody>
                  {missingRep.length === 0 ? (
                    <TableRow><TableCell colSpan={3} className="text-center py-8 text-muted-foreground">{t("reminders.allReportsSubmitted")}</TableCell></TableRow>
                  ) : (
                    missingRep.map((m, i) => (
              ...
              <Table>
                <TableHeader>
                  <TableRow className="hover:bg-transparent">
                    ...
                    <TableHead className="text-xs uppercase tracking-wider text-muted-foreground">{t("reminders.sent")}</TableHead>
                    <TableHead className="text-xs uppercase tracking-wider text-muted-foreground">{t("common.last")}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {logs.length === 0 ? (
                    <TableRow><TableCell colSpan={6} className="text-center py-8 text-muted-foreground">{t("reminders.noReminderLogs")}</TableCell></TableRow>
                  ) : (
                    logs.map((l) => (
                      <TableRow key={l.id} className="table-row-hover" data-testid={`log-${l.id}`}>
                        ...
                        <TableCell>
                          <Badge variant="outline" className={`text-xs ${l.type === "MissingAttendance" ? "text-amber-400 border-amber-500/30" : "text-blue-400 border-blue-500/30"}`}>
                            {l.type === "MissingAttendance" ? t("reminders.attendance") : t("reminders.report")}
                          </Badge>
                        </TableCell>
                        <TableCell className="font-mono text-xs text-primary">{l.project_code || "-"}</TableCell>
                        <TableCell>
                          <Badge variant="outline" className={`text-xs ${LOG_STATUS_COLORS[l.status] || ""}`}>{t(`reminders.logStatus.${l.status.toLowerCase()}`)}</Badge>
                        </TableCell>
                        <TableCell className="text-sm text-muted-foreground">{l.reminder_count}x</TableCell>
                        <TableCell className="text-xs text-muted-foreground">
                          {l.last_reminded_at ? formatTime(l.last_reminded_at) : "-"}
                        </TableCell>
                      </TableRow>
                    ))
                  )}
      ...
      <Dialog open={excuseOpen} onOpenChange={setExcuseOpen}>
        <DialogContent className="sm:max-w-[400px] bg-card border-border" data-testid="excuse-dialog">
          <DialogHeader>
            <DialogTitle>{t("reminders.excuseUser")}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 pt-2">
            <div className="space-y-2">
              <label className="text-sm text-muted-foreground">{t("workReports.reason")} *</label>
              <Input value={excuseReason} onChange={(e) => setExcuseReason(e.target.value)} placeholder={t("reminders.excusePlaceholder")} className="bg-background" data-testid="excuse-reason-input" />
            </div>
