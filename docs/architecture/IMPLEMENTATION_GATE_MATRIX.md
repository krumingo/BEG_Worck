...
| 032 | W0-03 | **A/B1/B2/B3/C(min)/UI MERGED — W0-03C UNIQUENESS IN REVIEW** | уникалните индекси (построени само след чист duplicate report върху възстановено копие — [бележка](W0-03C_UNIQUENESS_READINESS.md)), merge с preview и Approval (D), legacy миграция и isolation suite (E). Инвентарът (`W0-03_MASTER_DATA_INVENTORY_AND_CONTRACT.md`) показва 15 идентичностни колекции, нула уникални индекса върху master data, дублирани писачи на `persons`/`companies` и седем нескоупнати изтривания |
...
Няма оставащи бизнес решения. Wave 0 coding е започнал; W0-09A и W0-10A са затворени (20.09.2026). Текущата implementation стъпка е **W0-03 Master Data**: A–C(min) и офис екранът са merge-нати, **W0-03C уникалност е в ревю**, D и E не са започнати; нищо от W0-03 не е деплойвано (пълният ред е в [Implementation Waves](IMPLEMENTATION_WAVES.md)).
