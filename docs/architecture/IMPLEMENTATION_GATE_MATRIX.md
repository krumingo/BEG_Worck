...
| 002 | 100% | auth, JWT, single `user.role`, project-team checks | **W0-BLOCKER** | RoleAssignment + Permission Service + migration |
| 003 | 100% | offers, lines, versions, Excel import, extra-work drafts | **REFACTOR / READY-W1** | stable Line_ID, provenance, DQ/Approval, client/internal views |
...
| 007 | 100% | extra-work drafts and offer creation foundation | **READY-W1** | FLOW-003 stable identity + FLOW-034 + FLOW-005/client approval |
| 008 | 85% | P&L/full-cost/financial-result routes | **BUSINESS-OPEN** | close 3 dashboard/advance/drill-down decisions; source map |
...
| 014 | 100% | work_reports, daily_reports, work_logs, media | **W0/W1 REFACTOR** | no report without presence/SMR/time; side-work/change flow; migration |
...
| 019 | 100% | labor-by-SMR and paid-labor services | **READY-W2** | canonical FLOW-013/014 + FLOW-028 source-of-truth |
...
| 024 | 100% | overhead categories/costs/snapshots/allocations | **READY-W2** | canonical allocation rules and P&L source tests |
| 025 | 100% | files exist, but no full document-control domain | **READY-W1** | FLOW-016 first; document families/checklists/signature state |
...
| 028 | 100% | Pay Runs v3, slips, allocations, sync, audit checks | **REFACTOR / READY-W2** | FLOW-013/014; akord quantity rule; payment idempotency; approval |
...
| 031 | 100% | business agent-shапки, no central runtime orchestration | **READY-W3** | FLOW-030 tool layer + FLOW-002 + AuditEvent |
...
| 035 | 85% | subcontractor package and budgets, no generic package | **BUSINESS-OPEN** | close package-generation/UI decisions; generic WorkPackage model |
...
| 042 | 40% | legacy smoke script; no FLOW acceptance center | **BUSINESS-OPEN / W0** | test schema, CI, migration tests, forbidden scenarios, release linkage |
| 043 | 100% | architecture documentation | **DOC PASS** | enforce through ADRs, lint/checklists and code review |
| 044 | 100% | no proven standby/PITR/restore implementation | **OPS-W0** | version manifest, backups, immutable copy, restore drill |
| 045 | 95% | no command-center runtime | **BUSINESS-OPEN / READY-W3** | final intent→data→draft→confirm→action matrix |
| 046 | 75% | no client portal runtime | **BUSINESS-OPEN** | identity/magic link, visibility, communication, approval receipt |
| 047 | 100% | no generic managed package/bonus fund runtime | **READY-W2 after 035** | WorkPackage + budget agreement/version + bonus calculation/approval |
...
- FLOW-002 — RoleAssignment / Permission Service.
