...
| W0-03 Master Data | **A/B1/B2/B3/C(min)/UI MERGED — W0-03C UNIQUENESS IN REVIEW — NOT DEPLOYED** | contract [W0-03_MASTER_DATA_INVENTORY_AND_CONTRACT.md](W0-03_MASTER_DATA_INVENTORY_AND_CONTRACT.md) (PR #16); основа и инертен `off` (PR #17); pending mapping, човешки преглед, нормализация и alias резолюция без автоматичен merge (PR #18); офис екран „За мапване“, права по каталога и един отворен pending ред под конкурентност (PR #19 → `main` `2d03f8ac`). W0-03C уникалност — read-only duplicate report и index bootstrap, който отказва при дубликати — в Draft PR ([бележка](W0-03C_UNIQUENESS_READINESS.md)) | **нито един уникален индекс не е създаден**; duplicate report върху прясно възстановено копие още не е пуснат (задача за Крум); конкурентността и индексите не са доказани срещу реален MongoDB; остават D (merge с preview и Approval) и E (миграция). W0-03 не е деплойван, `MASTER_DATA_MODE` по подразбиране е `off` |
...
**Статус на реда към 21.09.2026:** (1) WAVE-PLAN-SYNC — DONE; (2) W0-09A — **MERGED** (`fdf4d59e`, 14.09.2026), но **не е деплойван** в production; (3) W0-10A — **DONE, PASS** (20.09.2026): реален архив възстановен и проверен изолирано, production непроменен — [отчет](../ops/W0-10A_RESTORE_PROOF_2026-09-20.md). (4) **W0-03 Master Data тече:** A, B1, B2, B3, минималният C и офис екранът са merge-нати (PR #16–#19, `main` `2d03f8ac`); **W0-03C уникалност е в ревю** — индекси се строят само след чист duplicate report и само върху disposable локален MongoDB; production не е пипан. D и E не са започнати. Следващите slice-ове започват само с отделна ASSIGNMENT.
