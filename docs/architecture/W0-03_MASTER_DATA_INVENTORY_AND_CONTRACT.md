...
- **Тестове:** миграция върху възстановено копие; isolation suite; regression срещу baseline;
  отрицателни тестове за изтриване на използван запис.
...
Три технически решения и **едно истинско бизнес решение**. Останалите са затворени в раздел 4.
...
### Q4 (бизнес решение) — `guest_name` при аванси

**Безопасен default, който предлагам:** свободният текст остава **вход / pending mapping** и
**не става автоматично официален person**. Авансът продължава да сочи текста, докато офисът
не го мапне към съществуващо лице или не създаде ново с потвърждение.

> ### 🔴 ЗАДАЧА ЗА КРУМ — решение за `guest_name`
>
> Днес `app/models/hr.py` позволява аванс или заем да се издаде на `guest_name` — свободен
> текст без `id` и без tenant. Такъв човек не може да бъде обединен, не влиза в справките
> като лице и не може да носи история.
>
> Въпросът за теб: **аванс на човек без официален Master запис допустим ли е изобщо?**
>
> - **Вариант А (моят default):** допустим, но текстът остава „за мапване"; офисът го
>   свързва с лице по-късно. Нищо не се блокира днес.
> - **Вариант Б:** не е допустим — аванс се издава само на официален person. По-чисто
>   финансово, но блокира текущ работен навик.
>
> Има финансови последици: при Вариант А, докато мапването не стане, сумата виси към низ.
