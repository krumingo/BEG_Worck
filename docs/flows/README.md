# BEG_Work — FLOW Documentation Index

> **Обхват:** FLOW-001–FLOW-049  
> **Канон:** отделните FLOW документи + Master Flow Register + последващи изрични решения на Крум  
> **Важно:** Business Lock и Implementation Gate са отделни статуси  
> **Проследимост:** всеки FLOW файл съдържа стандартна секция `Източници / сесии`  
> **Текущо:** 36 Business Locked, 12 активни незавършени, 1 legacy.

## Основни регистри

- [Master Flow Register](../project-recovery/2026-07-20/03_MASTER_FLOW_REGISTER_SEED.md)
- [Decision Register](../project-recovery/2026-07-20/04_DECISION_REGISTER_SEED.md)
- [Master Knowledge Base Seed](../project-recovery/2026-07-20/05_MASTER_KNOWLEDGE_BASE_SEED.md)

## Архитектура и готовност за програмиране

- [Architecture Index](../architecture/README.md)
- [Correction Pass — 20.07.2026](../architecture/CORRECTION_PASS_2026-07-20.md)
- [Cross-FLOW Code Audit](../architecture/CROSS_FLOW_AUDIT_2026-07-20.md)
- [Claude Cross-FLOW Logic Audit](../architecture/CLAUDE_CROSS_FLOW_LOGIC_AUDIT_2026-07-20.md)
- [FLOW-025 Business Close — 21.07.2026](../architecture/FLOW_025_BUSINESS_CLOSE_2026-07-21.md)
- [Implementation Gate Matrix](../architecture/IMPLEMENTATION_GATE_MATRIX.md)
- [Implementation Waves](../architecture/IMPLEMENTATION_WAVES.md)

## FLOW-001–FLOW-010

- [FLOW-001 — Обекти / клиенти / структура на проекта](FLOW-001.md)
- [FLOW-002 — Потребители / роли / права / достъп](FLOW-002.md)
- [FLOW-003 — Оферта / СМР редове / Excel-КСС / версии](FLOW-003.md)
- [FLOW-004 — Оферта → Акт → Остатък → Прогноза](FLOW-004.md)
- [FLOW-005 — Договори / анекси / договорна база](FLOW-005.md)
- [FLOW-006 — Фактури / аванси / плащания / ДДС](FLOW-006.md)
- [FLOW-007 — Допълнителни СМР / допълнителни оферти](FLOW-007.md)
- [FLOW-008 — Обект → приходи → разходи → печалба](FLOW-008.md)
- [FLOW-009 — Склад / наличности / материални движения](FLOW-009.md)
- [FLOW-010 — Клиенти / контакти / контрагенти](FLOW-010.md)

## FLOW-011–FLOW-020

- [FLOW-011 — Инструменти / машини / QR / активи](FLOW-011.md)
- [FLOW-012 — Логистика / доставки / курсове](FLOW-012.md)
- [FLOW-013 — Работници / присъствие / обект / СМР](FLOW-013.md)
- [FLOW-014 — Дневни отчети / снимки / доказателства](FLOW-014.md)
- [FLOW-015 — Dashboard / справки / аларми](FLOW-015.md)
- [FLOW-016 — File Registry / документи / файлове](FLOW-016.md)
- [FLOW-017 — Задачи / GitHub / Emergent / контрол](FLOW-017.md)
- [FLOW-018 — Quality / дефекти / гаранции — legacy MVP](FLOW-018.md)
- [FLOW-019 — СМР / труд / производителност / марж](FLOW-019.md)
- [FLOW-020 — Материали / заявки / доставки / фактури](FLOW-020.md)

## FLOW-021–FLOW-030

- [FLOW-021 — Подизпълнители / бригади / рейтинг](FLOW-021.md)
- [FLOW-022 — Цени труд София](FLOW-022.md)
- [FLOW-023 — Автоматично офериране / анализ цена](FLOW-023.md)
- [FLOW-024 — Режийни / Overhead](FLOW-024.md)
- [FLOW-025 — Договорни документи / версии / контрол](FLOW-025.md)
- [FLOW-026 — Аларми за риск / загуба / кеш](FLOW-026.md)
- [FLOW-027 — График / човекодни / прогрес / закъснение](FLOW-027.md)
- [FLOW-028 — Заплати / акорд / Pay Run / фишове](FLOW-028.md)
- [FLOW-029 — Архив снимки и видеа по обекти](FLOW-029.md)
- [FLOW-030 — BEG Brain / централен AI](FLOW-030.md)

## FLOW-031–FLOW-040

- [FLOW-031 — Модулни AI agent-шапки](FLOW-031.md)
- [FLOW-032 — Master Data / единна база](FLOW-032.md)
- [FLOW-033 — Data Quality Center](FLOW-033.md)
- [FLOW-034 — Approval Center](FLOW-034.md)
- [FLOW-035 — Work Package Engine](FLOW-035.md)
- [FLOW-036 — Object Timeline](FLOW-036.md)
- [FLOW-037 — Mobile Field App 2.0](FLOW-037.md)
- [FLOW-038 — Procurement Agent](FLOW-038.md)
- [FLOW-039 — Quality / Defects / Warranty](FLOW-039.md)
- [FLOW-040 — AI Audit Log / AuditEvent изглед](FLOW-040.md)

## FLOW-041–FLOW-049

- [FLOW-041 — Scenario / What-if Engine](FLOW-041.md)
- [FLOW-042 — Release / QA / Test Center](FLOW-042.md)
- [FLOW-043 — Architecture Decisions D-01–D-14](FLOW-043.md)
- [FLOW-044 — Disaster Recovery](FLOW-044.md)
- [FLOW-045 — AI Command Center](FLOW-045.md)
- [FLOW-046 — Client Portal](FLOW-046.md)
- [FLOW-047 — Managed Work Package / бонус](FLOW-047.md)
- [FLOW-048 — Resource Assignment](FLOW-048.md)
- [FLOW-049 — Marketplace / свободен капацитет](FLOW-049.md)

## Правило за редакция

Всяка следваща сесия трябва да обновява:

1. засегнатия FLOW документ;
2. секцията `Източници / сесии` в него;
3. Master Flow Register;
4. Decision Register, когато има ново архитектурно решение;
5. Session Archive / audit note;
6. Implementation/QA backlog, когато FLOW е готов за технически gate.

Преди merge се проверява:

- непрекъсната серия FLOW-001–049;
- `Източници / сесии` във всички 49 файла;
- съвпадение на проценти и оставащи точки между регистъра и отделните FLOW файлове;
- съвпадение на D-решенията между FLOW-043 и Decision Register;
- съвпадение на Gate Matrix/Waves с последния business-close pass.
